...
void printhelp(char *s) {
    ...
    printf("-p            ASM Parameter file[asmsearch.txt]\n");
    printf("-l            Left Lung Model[LungL.asm]\n");
    printf("-r            Right Lung Model[LungR.asm]\n");
    printf("-subsample    subsample image for ASM[4]\n");
	printf("-mask         Mask file name\n");
	printf("-overlay      Overlay file name\n");
	printf("-noorient     No reorientation in chesh_AP_PA\n");
    printf("\nExample:\n");
    printf("     %s  -i lung.tif -p ASMsearch.txt -l LungL.asm -r LungR.asm -mask mask.png -overlay overlay.tif -noorient -subsample 4\n");
    exit(1);
}
...
int main(int argc, char** argv) {
	...
	char *input=NULL;
	char *parameter="asmsearch.txt";
	char *LungL="LunL.asm";
	char *LungR="LungR.asm";
    ...
    p++;
    while (*p) {
        if(strcmp(*p,"-h")==0 || strcmp(*p,"-help")==0) printhelp(argv[0]);
        else if(strcmp(*p,"-i"         )==0) input = *(++p);
        else if(strcmp(*p,"-p"         )==0) parameter = *(++p);
        else if(strcmp(*p,"-l"         )==0) LungL = *(++p);
        else if(strcmp(*p,"-r"         )==0) LungR = *(++p);
        else if(strcmp(*p,"-overlay"   )==0) OverlayImage = *(++p);
        else if(strcmp(*p,"-mask"      )==0) MaskImage = *(++p);
		else if(strcmp(*p,"-noorient"  )==0) reorient = false;
        else if(strcmp(*p,"-subsample" )==0) subsample = atoi(*(++p));
		else if(strcmp(*p,"-v"         )==0) verbose = true;
		else {
...
//	printf("TIME2: %d\n",time);
    try {
        ...
        IemImage image = iemRead(input);
//		time = clock();
//		printf("TIME2: %d\n",time);

        Chest_PA_AP chest;
		if(verbose)chest.setVerbose(true);
		chest.setBoostFactor(0.10);
		chest.setReorientation(reorient);
...
//		printf("Filename:  %s\n",tmpnm.c_str());
		...
		if(!MaskImage) out1 = tmpnm + out1;
		else           out1 = MaskImage;
		if(!OverlayImage) out2 = tmpnm + out2;
		else              out2 = OverlayImage;

        bool rc;
        rc =  LeftLung.LoadModel(parameter, LungL);
        if (!rc) {
            printf("Error loading the left model or paramater file.\n");
            exit(1);
        }
        rc = RightLung.LoadModel(parameter, LungR);
        if (!rc) {
		...
		int Orow=image.rows(), Ocol=image.cols();
		if(subsample) image = iemSubsample(image,subsample,subsample);
...
//		printf("TIME2: %d\n",time);
		...
		int factor = 1;
		if(subsample) factor = subsample;
        IemPlane output(IemByte,Orow, Ocol);
        output = 0;
