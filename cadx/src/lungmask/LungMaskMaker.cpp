...
LungMaskMaker::LungMaskMaker(char* _paramFileName, char* _rightLungFileName, char* _leftLungFileName, int _subsample) {
	initialize();
	
	subsample = _subsample;
	
	strcpy(paramFileName, _paramFileName);
	strcpy(rightLungFileName, _rightLungFileName);
	strcpy(leftLungFileName, _leftLungFileName);
}


void LungMaskMaker::initialize() {
	subsample = 4;
	reorient = false;
	verbose = false;
	maxCodeValue = 4095;
...
IemTImage<unsigned char> LungMaskMaker::makeMask(IemTImage<short>& imgIn) {

	IemImage image = convertTo8Bit(imgIn);
	...
	bool rc;
	rc =  LeftLung.LoadModel(paramFileName, leftLungFileName);
	...
	rc = RightLung.LoadModel(paramFileName, rightLungFileName);
...
//    IemLinearArray spine      = chest.getSpineLine();
	...
	IemTPlane<byte> imgMask(output);

	return imgMask;
}


IemImage LungMaskMaker::convertTo8Bit(IemTImage<short>& img) {

	double k = (255.0 / (double)maxCodeValue);

	IemImage img8bit(IemByte, 1, img.rows(), img.cols());
...
vector<byte *> LungMaskMaker::Around_the_Points(IemTPlane<byte> &img, vector<byte *> points) {
...
void LungMaskMaker::FillShape(IemPlane output, int x0, int y0) {
...
void LungMaskMaker::DrawShape(IemPlane &output, vector<float> &ShapeV, int factor) {
