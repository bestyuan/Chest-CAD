...
* segmented lung region. Also, the watersheds are relabeled with the labels of
...
#define VERBOSE 0
#define MAXDETECTIONS 50
#define VERSION "2.0.0"
#define VERSIONDATE "November 21, 2002"
...
int main(int argc, char *argv[])
{
   ...
   unsigned char *image=NULL, *lungmaskim=NULL;
   unsigned short int *wshedim=NULL;
   char *feature_filename=NULL, *lungmask_filename=NULL,
      *gt_filename=NULL, filename[200], *origim_filename=NULL;
   ...
   int fullsizerows=0, fullsizecols=0;
   unsigned short int *fullsizeimage=NULL;
   float fullsizepixelspacing=0.0;

   void watershed_transform(unsigned char *image, int rows, int cols,
       unsigned short int **wshedim, unsigned char *lungmaskim);
   ...
   int simple_read_kesprio_image(char *filename, int *rows, int *cols,
      float *pixelsize, unsigned short int **image);
   ...
   ****************************************************************************/
   for(i=1,k=0;i<argc;i++){
      if(strcmp(argv[i], "-version") == 0){
         ...
         k++;
         if(k == 1) origim_filename = argv[i];
         if(k == 2) feature_filename = argv[i];
         else if(k == 3) lungmask_filename = argv[i];
      }
   }
   if(k != 3){
      ...
      printf("lung tissue). These files are produced by the program afumfeature. You\n");
      ...
      printf("\n<USAGE> %s [-version] origimage.img filename.feature.pgm filename.polyscan.pgm\n", argv[0]);
      exit(1);
   }
   ...
   ****************************************************************************/
   if(simple_read_kesprio_image(origim_filename, &fullsizerows, &fullsizecols, &fullsizepixelspacing, &fullsizeimage) == 0)
      return(0);
   if(fullsizepixelspacing == 0.0) fullsizepixelspacing = 0.168;
   free(fullsizeimage);
   fullsizeimage = NULL;
   ...
   ****************************************************************************/
   ...
   resolution = 1000.0 * ((float)fullsizecols / (float)cols) * fullsizepixelspacing;
   printf("The computed AFUM pixel spacing is %f microns.\n", resolution);

   /****************************************************************************
   * Read in the lungmask image.
   ****************************************************************************/
   if(strcmp(lungmask_filename, "none") == 0){
      printf("Because no mask was specified, the entire image is being used.\n");
      lungmaskim = (unsigned char *) calloc(rows*cols, sizeof(unsigned char));
      memset(lungmaskim, 1, rows*cols);
   }
   else{
      if(read_pgm_image(lungmask_filename, &lungmaskim, &rows, &cols) == 0) exit(1);
   }

   /****************************************************************************
   * Apply the watershed transform to segment the image.
   ****************************************************************************/
   watershed_transform(image, rows, cols, &wshedim, lungmaskim);
   ...
   ****************************************************************************/
   ...
   fprintf(fpdet, "# IMAGE FILENAME: %s\n", origim_filename);
   fprintf(fpdet, "# FEATURE FILENAME: %s\n", feature_filename);
   fprintf(fpdet, "# LUNGMASK FILENAME: %s\n", lungmask_filename);
    ...
    unsigned short int **wshedim, unsigned char *lungmaskim)
{
   ...
   ****************************************************************************/
   ...
   for(r=0,pos=0;r<rows;r++){
      for(c=0;c<cols;c++,pos++){
         if(lungmaskim[pos] != 0) chist[image[pos]]++;
      }
   }
      ...
      */
      ...
      for(pos=hoffset;pos<(hoffset+hnum);pos++){
         ...
         for(n=0;n<4;n++){
            ...
            if((rpos>=0)&&(cpos>=0)&&(rpos<rows)&&(cpos<cols)){
               ...
               if((lungmaskim[pprime]!=0) && ((im0[pprime] > 0) || (im0[pprime] == WSHED))){
                  imd[p] = 1;
      ...
      while(1){
         ...
         for(n=0;n<4;n++){
            ...
            if((rpos>=0)&&(cpos>=0)&&(rpos<rows)&&(cpos<cols)){
               pprime = rpos*cols+cpos;
               if((lungmaskim[pprime]!=0) && ((imd[pprime] < current_dist) && ((im0[pprime] > 0) || (im0[pprime] == WSHED)))){
                  if(im0[pprime] > 0){
	       ...
	       error_case = 3;
               ...
               for(n=0;n<4;n++){
                  ...
                  if((rpos>=0)&&(cpos>=0)&&(rpos<rows)&&(cpos<cols)){
                     ...
                     if((lungmaskim[pprimeprime]!=0) && (im0[pprimeprime] == MASK)){
   ...
   ****************************************************************************/
   ...
   for(r=0,pos=0;r<rows;r++){
      for(c=0;c<cols;c++,pos++){
	 if((lungmaskim[pos]!=0) && (im0[pos] == WSHED)){
      ...
      for(rr=(r-1);rr<=(r+1);rr++){
         for(cc=(c-1);cc<=(c+1);cc++){
            if((rr>=0)&&(cc>=0)&&(rr<rows)&&(cc<cols)){
               if((lungmaskim[rr*cols+cc]!= 0) && ((*wshedim)[rr*cols+cc] < minlabel)){
   ...
   ****************************************************************************/
   ...
   while(fifo_empty(qspos, qrpos) == 0){
      ...
      if((*wshedim)[pos] == thisdist){
         ...
         for(rr=(r-1);rr<=(r+1);rr++){
            for(cc=(c-1);cc<=(c+1);cc++){
               if((rr>=0)&&(cc>=0)&&(rr<rows)&&(cc<cols)&&(lungmaskim[rr*cols+cc]!=0)){
...
* outside the lung is relabeled with a very large label. All basins that
* do not exceed the lung boundary are relabeled to fill in the missing labels.
* This is a preet adhoc routine.
* Name: Michael Heath, University of South Florida
* Date: 11/5/99
*******************************************************************************/
int unliken_excessive_basins(unsigned short int *wsimage, int rows, int cols, unsigned char *lungmaskim)
{
   ...
   ****************************************************************************/
   ...
   for(r=0,p=0;r<rows;r++){
      for(c=0;c<cols;c++,p++){
         if(lungmaskim[p] == 0) basin[wsimage[p]] = 0;
      }
   }
